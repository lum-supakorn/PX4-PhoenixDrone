...
void TailsitterRateControl::mix(float fa, matrix::Vector3f momentum_ref, float* actuator_outputs){
	float outputs[4];
	outputs[0] = outputs[1] = outputs[2] = outputs[3] = 0.0f;
...
void TailsitterRateControl::get_omega2_l(float fa, matrix::Vector3f momentum_ref, float* omega2_l){
	float mx = momentum_ref(0);
	*omega2_l = (mx + 2*fa*_l)/(2*_kt*_l);
}

void
TailsitterRateControl::get_omega2_r(float fa, matrix::Vector3f momentum_ref, float* omega2_r){
	float mx = momentum_ref(0);
	*omega2_r = -(mx - 2*fa*_l)/(2*_kt*_l);
}

void TailsitterRateControl::get_delta_r(float fa, matrix::Vector3f momentum_ref, float* delta_r){
	float mx = momentum_ref(0);
...
void TailsitterRateControl::get_delta_l(float fa, matrix::Vector3f momentum_ref, float* delta_l){
	float mx = momentum_ref(0);
	float my = momentum_ref(1);
	float mz = momentum_ref(2);
