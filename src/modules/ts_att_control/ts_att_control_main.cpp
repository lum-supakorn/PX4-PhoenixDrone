...
private:
	...
	struct rate_ctrl_status_s 		rate_ctrl_status; /**< controller status */
	...
	matrix::Vector3f		_e_R_prev;      /**< previous attitude error in screw-axis convention */
	matrix::Vector3f		_e_R_int;       /**< attitude error integral */
	matrix::Vector3f		_rates_prev;	/**< angular rates on previous step */
	matrix::Vector3f		_rates_sp_prev; /**< previous rates setpoint */
	...
	struct {
		...
		matrix::Vector3f att_i;
		matrix::Vector3f att_d;
		matrix::Vector3f att_int_lim;			/**< integrator state limit for rate loop */
		matrix::Vector3f att_tc;
		matrix::Vector3f rate_tc;				/**< time constant for angular rate error */
		matrix::Vector3f rate_i;                 /**< rate integral gain*/
		matrix::Vector3f att_max;
		matrix::Vector3f rate_max;
		matrix::Vector3f rate_int_lim;			/**< rate integral limit*/
		matrix::Vector3f momt_const;				/** Momentum constants roll, pith, yaw **/
		...
		if (fds[0].revents & POLLIN) {
			...
			if (_v_control_mode.flag_control_attitude_enabled) {
				...
				if (_v_control_mode.flag_control_manual_enabled) {
					...
					_rates_sp = matrix::Vector3f(_manual_control_sp.y, -_manual_control_sp.x,
			...
			if (_v_control_mode.flag_control_rates_enabled) {
				...
				if (_controller_status_pub != nullptr) {
					orb_publish(ORB_ID(rate_ctrl_status), _controller_status_pub, &rate_ctrl_status);

				} else {
					_controller_status_pub = orb_advertise(ORB_ID(rate_ctrl_status), &rate_ctrl_status);
				}
			}
